...
# Challenge 2: Booting up the DiscordBot

TOKEN = "MTA3MzgwNTc4MTYyNDk0NjcwOA.GpsCsu.C2ZwGi9JfPwE2j-Eb0AM60OREgm2dQ8scbh-mg"  #insert your Token that you copied from Discord
...
Returns: no return value
'''    
@Bot.command(name="!! BLANK command !!", pass_context = True)
async def Call(ctx):
...
Returns: no return value
'''
@Bot.command(name="!! BLANK command1 !!", pass_context = True)
...
Returns: no return value
'''
@Bot.command(name="!! BLANK command2 !!",pass_context = True)
...
'''
@Bot.command(name="!! BLANK command3 !!",pass_context = True)
...
Returns: no return value
'''
@Bot.command(name="!! BLANK command4 !!",pass_context = True)
...
def get_temperature(city):
    ...
    url = "http://wttr.in/"+"!!BLANK ARGUMENT!!"+"?format=%t"
    # We are going to use the urlopen(url) function to obtain the url
    URLPage = "!! BLANK function !!"
...
async def on_message(message):
    ...
    if(message.content.startswith("BLANK Command")):
        # check if there is a city written as input
        if(message.content == "  same BLANK Command"):
            #let the Bot tell the user to put a city; use await message.channel.send() to let the Bot speak 
            
            return #"DO NOT DELETE"
        daily_forecast = get_temperature(message.content.split("!! same BLANK command here!! ", 1)[1])
        await message.channel.send(message.content.split("!! and BLANK here!!  ", 1)[1]+":"+ daily_forecast) #Bot-output statement
